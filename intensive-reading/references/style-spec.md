...
`SZ_COVER_EYEBROW`（20）**只用于 `# ` 一级标题那一行**。封面区其余加粗行（单元名｜板块之类的标签行）仍是 21——它们在原件里没有对应角色，按正文字号处理，只借用眉标的间距。
...
| NOTE | `#8B1E1E` | 模块标题（词汇注释 / 短语与固定搭配 / 长难句拆解 / 语法聚焦 / 精读）、`▶` 词条头 |
...
|---|---|---|---|---|---|---|
| 封面眉标〔`# ` 一级标题〕 | 0/40/— | | | | 居中 | Calibri+YaHei b · HEAD · 20 |
| 封面标签行〔单元名｜板块〕 | 0/40/— | | | | 居中 | Calibri+YaHei b · HEAD · 21 |
...
| 模块标题〔词汇注释 等〕 | 70/24/— | | | | keepNext | Calibri+SemiBold b · NOTE |
...
模块标题只有两档几何：`📖 精读` 走带底色的方框那一档，**其余 `###` 模块标题共用
「模块标题〔词汇注释 等〕」那一行**，加模块不必加规格。目前用到的：
✦ 表达清单 / 📖 词汇注释 / 🔗 短语与固定搭配 / 🔍 长难句拆解 / 🧩 语法聚焦（教材侧），
🎓 IELTS 写作迁移 / 📈 Task 1 迁移（外刊侧）。

外刊词条里的 `▶ 外刊写作赏析` 与 `▶ 🎓 IELTS 使用提醒` **不是 `###`**，它们是
`▶ 词条头 / 子标记` 那一档（暗红 keepNext，无缩进）——原件就是这么排的。
...
4. **紧跟文字行的 `- ` 列表 → 真列表。** 标准 Markdown 要求列表前有空行，
   否则 `常见搭配：` + 紧接的 `- xxx` 会被并进同一段，渲染成一行连字符。
